extends Node
## 비행기 움직임 실행 컴포넌트(MovementComponent). 부모 Aircraft의 명령(=FSM이 결정한 것)/설정을
## 읽어 속도 관성(가속/감속) + 회전 + 전진을 매 물리 프레임 부모에 반영한다. 명령/설정 관리는 Aircraft가 한다.
...
func _physics_process(delta: float) -> void:
	var command: Aircraft.Command = _aircraft.active_command()

	var target_speed: float = _aircraft.max_speed if command == Aircraft.Command.ADVANCE else 0.0
	...
	if command == Aircraft.Command.TURN_LEFT:
		turn_direction = 1.0
	elif command == Aircraft.Command.TURN_RIGHT:
		turn_direction = -1.0
